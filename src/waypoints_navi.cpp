...
bool WaypointsGoalNode::init()
{
  ...
  status_pub_  = nh.advertise<waypoint_nav::NavigationControlStatus>("nav_ctrl_status", 1, true);

  return true;
}

void WaypointsGoalNode::waypointsCB(const waypoint_nav::WaypointList::ConstPtr& wps)
{
  wp_list_ = *wps;
  ROS_INFO_STREAM("Received " << wp_list_.waypoints.size() << " way points.");
}

void WaypointsGoalNode::trajectoriesCB(const waypoint_nav::TrajectoryList::ConstPtr& trajs)
{
  traj_list_ = *trajs;
  ROS_INFO_STREAM("Received " << traj_list_.trajectories.size() << " trajectories.");
}

void WaypointsGoalNode::navCtrlCB(const waypoint_nav::NavigationControl::ConstPtr& nav_ctrl)
{
  if (nav_ctrl->control == waypoint_nav::NavigationControl::STOP)
  {
    if ((state_ == START) || (state_ == ACTIVE))
    {
      ...
      idle_status_update_sent_ = false;
      state_ = IDLE;
      publishStatusUpdate(waypoint_nav::NavigationControlStatus::CANCELLED);
    }
    else
    {
  ...
  else if ((nav_ctrl->control == waypoint_nav::NavigationControl::START))
  {
...
bool WaypointsGoalNode::cancelAllGoals(double timeout)
{
      ...
      (goal_state != actionlib::SimpleClientGoalState::PREEMPTED))
  {
    ...
    publishStatusUpdate(waypoint_nav::NavigationControlStatus::ERROR);
    return true;
  }
...
//        mb_goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(0.0);  // TODO use the heading from robot loc to next (front)
        ...
        publishStatusUpdate(waypoint_nav::NavigationControlStatus::RUNNING);
...
//        }
    ...
    else if (state_ == ACTIVE)
    {
          ...
          (goal_state == actionlib::SimpleClientGoalState::PREEMPTED))
      {
        ...
        if (waypoints_it_ < (waypoints_.end() - 1))
        {
          ...
          WaypointsGoalNode::pose2tf(mb_goal.target_pose, goal_gb);
          double distance = WaypointsGoalNode::distance2D(robot_gb, goal_gb);
    ...
    {
      // publish update
      publishStatusUpdate(waypoint_nav::NavigationControlStatus::COMPLETED);
      idle_status_update_sent_ = false;
      state_ = IDLE;
    }
    else // IDLE
    {
      ...
      {
        publishStatusUpdate(waypoint_nav::NavigationControlStatus::IDLING);
...
void WaypointsGoalNode::pose2tf(const geometry_msgs::Pose& pose, tf::Transform& tf)
{
...
void WaypointsGoalNode::pose2tf(const geometry_msgs::PoseStamped& pose, tf::StampedTransform& tf)
{
...
void WaypointsGoalNode::tf2pose(const tf::Transform& tf, geometry_msgs::Pose& pose)
{
...
double WaypointsGoalNode::distance2D(const tf::Transform& a, const tf::Transform& b)
{
  return distance2D(a.getOrigin(), b.getOrigin());
}


double WaypointsGoalNode::distance2D(double x, double y)
{
  return std::sqrt(std::pow(x, 2) + std::pow(y, 2));
}

double WaypointsGoalNode::distance2D(const tf::Point& p)
{
  return std::sqrt(std::pow(p.x(), 2) + std::pow(p.y(), 2));
}

double WaypointsGoalNode::distance2D(double ax, double ay, double bx, double by)
{
  return std::sqrt(std::pow(ax - bx, 2) + std::pow(ay - by, 2));
}

double WaypointsGoalNode::distance2D(const tf::Point& p1, const tf::Point& p2)
{
  return std::sqrt(std::pow(p2.x() - p1.x(), 2) + std::pow(p2.y() - p1.y(), 2));
}

double WaypointsGoalNode::distance2D(geometry_msgs::Point a, geometry_msgs::Point b)
{
  return distance2D(tf::Vector3(a.x, a.y, a.z), tf::Vector3(b.x, b.y, b.z));
}

double WaypointsGoalNode::distance2D(geometry_msgs::Pose a, geometry_msgs::Pose b)
{
...
{
  waypoint_nav::NavigationControlStatus msg;
  if (status == waypoint_nav::NavigationControlStatus::IDLING)
  {
    msg.status = waypoint_nav::NavigationControlStatus::IDLING;
    msg.status_desc = "Idling";
    status_pub_.publish(msg);
  }
  else if (status == waypoint_nav::NavigationControlStatus::RUNNING)
  {
    msg.status = waypoint_nav::NavigationControlStatus::RUNNING;
    msg.status_desc = "Navigating to way point.";
    status_pub_.publish(msg);
  }
  else if (status == waypoint_nav::NavigationControlStatus::PAUSED)
  {
    msg.status = waypoint_nav::NavigationControlStatus::PAUSED;
    msg.status_desc = "Navigation on hold.";
    status_pub_.publish(msg);
  }
  else if (status == waypoint_nav::NavigationControlStatus::COMPLETED)
  {
    msg.status = waypoint_nav::NavigationControlStatus::COMPLETED;
    msg.status_desc = "Reached final destination.";
    status_pub_.publish(msg);
  }
  else if (status == waypoint_nav::NavigationControlStatus::CANCELLED)
  {
    msg.status = waypoint_nav::NavigationControlStatus::CANCELLED;
    msg.status_desc = "Navigation cancelled.";
    status_pub_.publish(msg);
  }
  else if (status == waypoint_nav::NavigationControlStatus::ERROR)
  {
    msg.status = waypoint_nav::NavigationControlStatus::ERROR;
